...
{ 
    A_STATUS error = ENET_mediactl (g_Enet_handle,ENET_MEDIACTL_VENDOR_SPECIFIC,inout_param);
...
A_STATUS ath_set_mode(A_UINT32 device,char* mode)
{
    ...
    else
    {
        ...
        error = ENET_mediactl (g_Enet_handle,ENET_SET_MEDIACTL_MODE,&inout_param);
        if (ENET_OK != error)
        {
            return error;
        }
...
A_STATUS ath_send_wep_param(ENET_WEPKEYS* wep_param){
  return(ENET_mediactl (g_Enet_handle,ENET_SET_MEDIACTL_ENCODE,wep_param));
}
...
A_INT32 NetConnect(ATH_NETPARAMS *pNetparams)
{
	...
	do{
		...
		{	
	    	...
	    	ATH_SET_ESSID (DEMOCFG_DEFAULT_DEVICE, (char*)pNetparams->ssid);
			...
			if(pNetparams->sec_type == ENET_MEDIACTL_SECURITY_TYPE_WPA2){
		        ...
		        if(A_OK != ENET_mediactl (g_Enet_handle,ENET_MEDIACTL_VENDOR_SPECIFIC,&ath_param)){
		        	break;
		        }
		        strcpy(ver,"wpa2");
		        ATH_SET_SEC_TYPE (DEMOCFG_DEFAULT_DEVICE,ver); 
				
		        if(strlen((char*)pNetparams->u.passphrase) != 64)
				{								
					ATH_SET_PASSPHRASE (DEMOCFG_DEFAULT_DEVICE,(char*)pNetparams->u.passphrase);
				}
				else
				{
				...
				ATH_SET_SEC_TYPE (DEMOCFG_DEFAULT_DEVICE,"wep");
				set_wep_keys (DEMOCFG_DEFAULT_DEVICE,(char*)pNetparams->u.wepkey,
		    ...
		    ATH_COMMIT(DEMOCFG_DEFAULT_DEVICE);
		    status = 0;
...
A_INT32 ath_reg_query( A_INT32 argc, char* argv[])
{
	...
	do{
		...
		error = ENET_mediactl (g_Enet_handle, ENET_MEDIACTL_VENDOR_SPECIFIC, &ath_param);
...
{	
   ...
   /*Check if driver is loaded*/
    ...
    error = ENET_mediactl (g_Enet_handle,ENET_GET_MEDIACTL_POWER,&inout_param);
    if (A_OK != error)
    {
        return error;
    }
...
A_INT32 wmi_set_scan(A_UINT32 dev_num)
{
    ...
    if (dev_num < IPCFG_DEVICE_COUNT)
    {
        ...
        error = ENET_mediactl (g_Enet_handle,ENET_SET_MEDIACTL_SCAN,NULL);
        if (A_OK != error)
        {
            return error;
        }

        error = ENET_mediactl (g_Enet_handle,ENET_GET_MEDIACTL_SCAN,&param);
        if (A_OK != error)
        {
            return error;
        }
...
void pushButtonWPS(void)
{
    ...
    if(StartWPS(&netparams, 0,NULL) == 0)
    {
...
void SystemReset()
{
	...
	ath_param.length = sizeof(value);
        ...
        while((error = ENET_mediactl (g_Enet_handle,ENET_MEDIACTL_IS_INITIALIZED,&dev_status))!=A_OK)
        {
...
A_INT32 wmi_ping(A_INT32 argc, char* argv[] ){
    ...
    for(index = 1; index < argc ; index++)
    {
        ...
        {
            error=inet_aton(argv[index], (in_addr *)&hostaddr);
            if (error != -1)
            {
                ...
                if(ip_get_addr_for_host_name(argv[index],&hostaddr,NULL,2) != A_OK)
#endif
                {  
    ...
    {
        error =t_ping((void*) g_Enet_handle,hostaddr,size);
...
A_INT32 wmi_ping6(A_INT32 argc, char* argv[] ){
    ...
    for (index = 0; index < count; index++)
    {
        error =t_ping6((void*) g_Enet_handle,ip6addr,size);
